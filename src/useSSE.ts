...
export const useSSE = <ReturnData = unknown, Error = unknown>(
	...
	const contextId = dataCtx.id;
	const internalReference = internalCtx.effects[effectId];
	const [data, setData] = useState<unknown>(
		internalReference?.data || getServerSideData(effectId, contextId)?.data || null,
	);
	const [error, setError] = useState(
		internalReference?.error || getServerSideData(effectId, contextId)?.error || null,
	);
